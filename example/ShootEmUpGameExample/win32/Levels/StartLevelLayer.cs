...
namespace ShootEmUpGameExample
{
	...
	public class StartLevelLayer : CSKSeuGameLayer
	{
		...
		protected override void RunningOnNewWindow(CCSize WinSize)
		{

			base.RunningOnNewWindow(WinSize);
			...
			(starsEffect) =>
			{
				starsEffect.Position = new CCPoint(WinSize.Width, WinSize.Height / 2);
				starsEffect.PositionVar = new CCPoint(0, WinSize.Height / 2);
			...
			_galaxy = AddParallaxBackground("_galaxy", @"bg_galaxy-hd.png", 0, new CCPoint(0.015f, 0), new CCPoint(WinSize.Width + 200, 0), _backgroundNode);
			_spacialanomaly = AddParallaxBackground("_spacialanomaly", @"bg_spacialanomaly-hd.png", 0, new CCPoint(0.015f, 0), new CCPoint(WinSize.Width + 900, WinSize.Height * 0.3f), _backgroundNode);
			_spacedust1 = AddParallaxBackground("_spacedust1", @"bg_front_spacedust-hd.png", 0, new CCPoint(0.1f, 0), new CCPoint(WinSize.Width, WinSize.Height / 2), _foregroundNode);
			...
			// 7) Add space ship
			InitializePlayer(new Hero(), _bachShips, new CCPoint(WinSize.Width * 0.1f, WinSize.Height * 0.5f));
			Player.RotationX += 90;
			Player.RotationY += 90;
			...
			if (status == SneakyButtonStatus.Press)
			{
				...
				if (id == 1)
				{
					...
					shipLaser.RunAction(new CCSequence(
						new CCMoveBy(0.5f, new CCPoint(Director.WindowSizeInPixels.Width + 50, 0)),
		...
		public override void Update(float dt)
		{
			...
			if (curTime > _nextAsteroidSpawn)
			{
				...
				asteroid.RunAction(new CCSequence(
					   new CCMoveTo(CCRandom.GetRandomFloat(4.5f, 7), new CCPoint(-asteroid.TextureRect.Size.Width, asteroid.PositionY + CCRandom.GetRandomFloat(-50, 50))),
					   new CCCallFuncN(SetInvisible)
					   )
					   );
